...
public class RedirectionManager : MonoBehaviour {
    ...
    public enum RedirectorChoice { None, S2C, S2O, Zigzag, ThomasAPF, MessingerAPF, DynamicAPF, DeepLearning, PassiveHapticAPF , VisPoly, OpportunityAware};
    public enum ResetterChoice { None, TwoOneTurn, APF, OpportunityAware };
    ...
    void Awake()
    {
        ...
        if (globalConfiguration.movementController != GlobalConfiguration.MovementController.HMD)
        {
            ...
            // 在HMD模式下，将avatar身体设置为"Avatar"层
            // 这样可以在第一人称视图中通过摄像机的剔除遮罩来隐藏身体
            if (body != null && bodyHeadFollower != null)
            {
                // 确保HeadFollower创建了avatar可视化
                bodyHeadFollower.CreateAvatarViualization();
                
                // 设置avatar身体的层级
                bodyHeadFollower.SetAvatarBodyLayer();
                
                // 查找HMD相机并设置剔除遮罩
                var cameraRig = transform.Find("[CameraRig]");
                if (cameraRig != null)
                {
                    var cameras = cameraRig.GetComponentsInChildren<Camera>(true);
                    foreach (var cam in cameras)
                    {
                        // 剔除Avatar层，这样在第一人称视图中就看不到自己的身体
                        cam.cullingMask &= ~(1 << LayerMask.NameToLayer("Avatar"));
                    }
                }
            }
        }
    ...
    public System.Type RedirectorChoiceToRedirector(RedirectorChoice redirectorChoice)
    {
        switch (redirectorChoice)
        {
            ...
            case RedirectorChoice.OpportunityAware:
                return typeof(OpportunityAwareRDWRedirector);
        }
        return typeof(NullRedirector);
    }
    public static RedirectorChoice RedirectorToRedirectorChoice(System.Type redirector)
    {
        ...
        else if (redirector.Equals(typeof(OpportunityAwareRDWRedirector)))
            return RedirectorChoice.OpportunityAware;
        return RedirectorChoice.None;
    }
    public static System.Type DecodeRedirector(string s)
    {
        switch (s.ToLower())
        {
            ...
            case "passivehapticapf":
                return typeof(PassiveHapticAPF_Redirector);
            case "opportunityaware":
            case "opportunityawarerdw":
                return typeof(OpportunityAwareRDWRedirector);
            default:
                return typeof(NullRedirector);
        }
    ...
    public static System.Type ResetterChoiceToResetter(ResetterChoice resetterChoice)
    {
        switch (resetterChoice)
        {
            ...
            case ResetterChoice.OpportunityAware:
                return typeof(OpportunityAwareResetter);
        }
        return typeof(NullResetter);
    }
    public static ResetterChoice ResetterToResetChoice(System.Type reset)
    {
        ...
        else if (reset.Equals(typeof(OpportunityAwareResetter)))
            return ResetterChoice.OpportunityAware;
        return ResetterChoice.None;
    }
    ...
    {            
        ...
        // 通知统计记录器重置开始
        globalConfiguration.statisticsLogger.Event_Reset_Started(movementManager.avatarId);
    }
    ...
    {                
        ...
        ifJustEndReset = true;
        // 通知统计记录器重置结束
        globalConfiguration.statisticsLogger.Event_Reset_Completed(movementManager.avatarId);
    }
